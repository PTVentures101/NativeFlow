...
export function QueryInput({
  ...
  const handleSpeechResult = useCallback(
    (transcript: string) => onChange(transcript),
    [onChange]
  );

  const { isListening, toggle: toggleMic } = useSpeechInput({
        ...
        <div className="flex items-center gap-2">
            ...
            <button
              ...
              className={`
                ...
                ${isListening
                  ...
                  : "text-[#86868b] hover:text-[#1d1d1f] dark:hover:text-[#f5f5f7] hover:bg-black/5 dark:hover:bg-white/[0.08]"
                }
              `}
            >
          ...
          <button
            ...
            className={`
              ...
              ${canSubmit
                ? "bg-indigo-500 hover:bg-indigo-600 active:bg-indigo-700 text-white shadow-sm shadow-indigo-500/20 active:scale-95"
                : "bg-indigo-500/[0.10] dark:bg-indigo-400/[0.10] text-indigo-400/70 dark:text-indigo-400/60 cursor-not-allowed"
              }
            `}
          >
            {isLoading ? <><Spinner /> {loadingLabel}</> : <>{submitLabel} <ArrowIcon /></>}
          </button>
        </div>
      </div>
    </div>
